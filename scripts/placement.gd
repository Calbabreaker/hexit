...
onready var placement_ps := $PlacementPS
...
func _process(delta: float) -> void:
	...
	ghost_piece.modulate.r = 10 if Global.grid.has(mouse_hex_pos) else 1 # Redden sprite when piece already exists there

func _input(event: InputEvent) -> void:
	# If the mouse is pressed and a piece hasn't been placed at the position yet
	if event.is_action_pressed("ui_click") && !Global.grid.has(mouse_hex_pos):
